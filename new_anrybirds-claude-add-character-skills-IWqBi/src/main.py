...
try:
    ...
    # 加载背景和UI
    background2 = pygame.image.load("../resources/images/background1.png").convert_alpha()
...
def check_win_condition():
    ...
    # ====== 重点改这里 ======
    # 没鸟可用了
    no_birds_left = (level.number_of_birds <= 0)
    # 场上是否还有“有效鸟”
    active_birds = [
        b for b in birds 
        if b.body.velocity.length > 20      # 还有动能
        and b.body.position.y > 80          # 没掉地
        and -50 < b.body.position.x < 1250  # 在场内
    ]
    # 场景是否整体稳定
    scene_quiet = all(
        obj.body.velocity.length < 8
        for obj in pigs + columns + beams
    )
    # ★★★★★ 核心结算条件 ★★★★★
    if no_birds_left and len(active_birds) == 0:
        # 立刻开始强制倒计时
        ...
        # 2. 最多等2秒，无论如何都结算
        if time.time() - force_end_timer_start > 2.0:
            return 'defender_wins'
    else:
        # 还有鸟可发挥作用 → 重置
        force_end_timer_start = 0
